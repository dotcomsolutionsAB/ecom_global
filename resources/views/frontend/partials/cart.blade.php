    ...
    <ul class="h-250px overflow-auto c-scrollbar-light list-group list-group-flush">
      ...
      @foreach ($cart as $key => $cartItem)
        @php
          ...
          $total = $total + cart_product_price($cartItem, $product, false) * ($cartItem['is_carton'] ? $cartItem['quantity'] * $product_stock->piece_per_carton : $cartItem['quantity']);
        @endphp
        @if ($product != null)
              ...
              <a href="{{ route('product', $product->slug) }}" class="text-reset d-flex align-items-center flex-grow-1">
                ...
                <span class="minw-0 pl-2 flex-grow-1">
                  ...
                  <span class="">{{ $cartItem['quantity'] }}
                    ...
                    x</span>
                  <span
                    class="">{{ $cartItem['is_carton'] ? single_price(cart_product_price($cartItem, $product, false) * $product_stock->piece_per_carton) : cart_product_price($cartItem, $product) }}</span>
                </span>
              </a>
              <span class="">
